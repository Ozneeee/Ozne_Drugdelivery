...
AddEventHandler('esx_brinks:GiveItem', function()
  ...
  if Quantity >= 20 then
    ...
    TriggerClientEvent('esx:showNotification', _source, 'Vous avez récupérer ~g~x' .. amount .. ' sachet de drogue')
  end
...
local function Vente(source)
  ...
  SetTimeout(Config.Zones.Vente.ItemTime, function()
    ...
    if Quantity < Config.Zones.Vente.ItemRemove then
      TriggerClientEvent('esx:showNotification', _source, '~r~Vous n\'avez plus de '..Config.Zones.Vente.ItemRequires_name..' à déposer. ~r~Retournez voir le ~g~Gérant ! ')
